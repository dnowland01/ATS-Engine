...
class InterviewHandler
{
    ...
    public function process(string $text): void
    {
        ...
        if ($dupIndex !== null) {
            $table[$dupIndex][10] = $this->status->determine('interview');
            $table[$dupIndex][11] = $this->notes->generate('interview');
            $this->tables->save('data/main_table.csv', $table);
            $this->summary->log("Interview updated for {$company} - {$role}");
            return;
        }
        ...
        $row = [
            ...
            "UK",
            date('d-M'),
            $this->status->determine('interview'),
            $this->notes->generate('interview'),
            0,
        ...
        $this->summary->log("Interview added for {$company} - {$role}");
    }
